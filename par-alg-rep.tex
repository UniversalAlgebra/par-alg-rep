...
We call $\theta$ and $f$
\begin{itemize}
\item \defn{weakly compatible} provided $\forall a_i \mathrel{\theta} b_i$,
if $f(\ba)$ and $f(\bb)$ are defined, then $f(\ba) \mathrel{\theta} f(\bb)$.
\item \defn{strongly compatible} provided $\forall a_i \mathrel{\theta} b_i$,
...
on $A$ a \defn{strong (weak) congruence relation} of $\A$ if it is 
